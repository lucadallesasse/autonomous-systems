...
bool BasicPlanner::planTrajectory(const Eigen::VectorXd& goal_pos,
    ...
    for (int i = 1; i < 21; i++) {
        ...
        //Load positions
        if (!nh_.getParam(ros::this_node::getName() + "/waypoints"+ "/p" + std::to_string(i) + "/x", px)){
            ROS_WARN("Param position not found");
        }
        if (!nh_.getParam(ros::this_node::getName() + "/waypoints"+ "/p" + std::to_string(i) + "/y", py)){
            ROS_WARN("Param position not found");
        }
        if (!nh_.getParam(ros::this_node::getName() + "/waypoints"+ "/p" + std::to_string(i) + "/z", pz)){
            ROS_WARN("Param position not found");
        }
        ...
        //Load velocities
        if (!nh_.getParam(ros::this_node::getName() + "/waypoints"+ "/v" + std::to_string(i) + "/x", vx)){
            //ROS_WARN("Param velocity not found");
        }
        if (!nh_.getParam(ros::this_node::getName() + "/waypoints"+ "/v" + std::to_string(i) + "/y", vy)){
            //ROS_WARN("Param velocity not found");
        }
        if (!nh_.getParam(ros::this_node::getName() + "/waypoints"+ "/v" + std::to_string(i) + "/z", vz)){
            //ROS_WARN("Param velocity not found");
        }
        ...
        //Load accelerations
        if (!nh_.getParam(ros::this_node::getName() + "/waypoints"+ "/a" + std::to_string(i) + "/x", ax)){
            //ROS_WARN("Param acceleration not found");
        }
        if (!nh_.getParam(ros::this_node::getName() + "/waypoints"+ "/a" + std::to_string(i) + "/y", ay)){
            //ROS_WARN("Param acceleration not found");
        }
        if (!nh_.getParam(ros::this_node::getName() + "/waypoints"+ "/a" + std::to_string(i) + "/z", az)){
            //ROS_WARN("Param acceleration not found");
        }
